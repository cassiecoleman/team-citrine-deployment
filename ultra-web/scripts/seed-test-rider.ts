...
import { ensureSeededRiderRows } from '../src/lib/seed-test-rider'
...
async function seedRider() {
  ...
  if (existingUser) {
    const repairResult = await ensureSeededRiderRows(supabase, existingUser.id, {
      name: RIDER.name,
      phone: RIDER.phone,
    })
    ...
    console.log(`  [done] ${RIDER.email} — existing auth user verified (${existingUser.id})`)
  ...
  const repairResult = await ensureSeededRiderRows(supabase, userId, {
    name: RIDER.name,
    phone: RIDER.phone,
  })
